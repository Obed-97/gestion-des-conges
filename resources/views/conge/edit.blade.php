...
<div class="m-t-40 card-box">
    <div class="text-center">
        <img src="{{asset('assets/images/agetic.png')}}" width="100" height="70">
        <h4 class=" font-bold m-b-0">Joindre une note de service</h4>
    </div>
    <form method="POST" action="{{route('Conge.autorise', $conge->id)}}" enctype="multipart/form-data">
     @csrf
    {{method_field('PUT')}}
    <div class="col-xs-12 text-center">
        <img src="/assets/images/users/{{$conge->user['photo']}}" class="mb-2 rounded-circle" width="120" height="120"  alt="">
       <h2 class="m-r-5">{{$conge->user['prenom']}} {{$conge->user['nom']}}</h2>
       <h4 class="m-r-5"> Poste : {{$conge->user->role['name']}}</h4>
                              
       <h4 class="m-r-5">{{$conge->type_conge['lib_type_conge']}}: {{$conge->nbjrs_conge}} jour(s) 
    ...
    <div class="p-20">
        ...
        <div class="form-group text-center m-t-40 mb-0">
            <button type="submit" class="btn btn-primary btn-block  waves-effect waves-light"><i class="fa fa-paperclip"></i> Joindre</button>
        </div>
    </div>
    </form>
</div>
...
<div class="m-t-40 card-box">
    <div class="text-center">
        <img src="{{asset('assets/images/agetic.png')}}" width="100" height="70">
        <h4 class=" font-bold m-b-0">Traiter le congé</h4>
    </div>
    <div class="p-20">
        <div class="col-xs-12 text-center">
                <img src="/assets/images/users/{{$conge->user['photo']}}" class="mb-2 rounded-circle" width="120" height="120"  alt="">
        ...
        <div class="col-xs-12 mb-5 text-center">
               <h5 class="m-r-5"><a href="{{route('ouvrir', $conge->id)}}"><i style="font-size: 50px; align-content: center; color: red;" class="fa fa-file-pdf-o"></i></a></h5>
        </div>
            <div class="row">
                <div class="col-md-3"></div>
                <div class="col-md-6  d-flex">
                    <form method="POST" action="{{route('Conge.valide', $conge->id)}}" enctype="multipart/form-data">
                        @csrf
                        {{method_field('PUT')}}
                        <button href="" class="btn  btn-primary  waves-effect waves-light m-b-5  mr-2"><i class="fa fa-check"></i> Validé</button>
                    </form>
                    <form method="POST" action="{{route('Conge.refuse', $conge->id)}}" enctype="multipart/form-data">
                        @csrf
                        {{method_field('PUT')}}
                        <button type="submit" class="btn  btn-danger  waves-effect waves-light m-b-5">
                         <b>X</b> Refusé
                        </button>
                    </form>
                </div>
...
<div class="m-t-40 card-box">
    ...
    <div class="p-20">
        <div class="col-xs-12 text-center">
               ...
               @if($conge->statut == 'Nouvelle demande')
                ...
                @elseif($conge->statut == 'Validé par la section') 
                  <span class="badge badge-pink">{{$conge->statut}}</span></h4>
                @elseif($conge->statut == 'Validé par la division')
                  <span class="badge badge-secondary">{{$conge->statut}}</span></h4>
                @else
                  <span class="badge badge-danger">{{$conge->statut}}</span></h4>
                @endif
            </h4>
        ...
        <div class="col-md-3"></div>
        <div class="col-md-6  d-flex">
            <form method="POST" action="{{route('Conge.traite', $conge->id)}}" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <button href="" class="btn  btn-primary  waves-effect waves-light m-b-5  mr-2"><i class="fa fa-check"></i> Validé</button>
            </form>
            <form method="POST" action="{{route('Conge.rejete', $conge->id)}}" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <button type="submit" class="btn  btn-danger  waves-effect waves-light m-b-5">
                 <b>X</b> Refusé
...
<div class="m-t-40 card-box">
    ...
    <div class="p-20">
        <div class="col-xs-12 text-center">
               ...
               @if($conge->statut == 'Nouvelle demande')
                ...
                @elseif($conge->statut == 'Validé par le service') 
                  <span class="badge badge-primary">{{$conge->statut}}</span></h4>
                @elseif($conge->statut == 'Validé par la division')
                  <span class="badge badge-secondary">{{$conge->statut}}</span></h4>
                @else
                  <span class="badge badge-danger">{{$conge->statut}}</span></h4>
        ...
        <div class="col-md-3"></div>
        <div class="col-md-6  d-flex">
            <form method="POST" action="{{route('Conge.accepte', $conge->id)}}" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <button href="" class="btn  btn-primary  waves-effect waves-light m-b-5  mr-2"><i class="fa fa-check"></i> Validé</button>
            </form>
            <form method="POST" action="{{route('Conge.invalide', $conge->id)}}" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <button type="submit" class="btn  btn-danger  waves-effect waves-light m-b-5">
                 <b>X</b> Refusé
...
<div class="m-t-40 card-box">
    ...
    <div class="p-20">
        ...
        <div class="col-md-3"></div>
        <div class="col-md-6  d-flex">
            <form method="POST" action="{{route('Conge.favorable', $conge->id)}}" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <button href="" class="btn  btn-primary  waves-effect waves-light m-b-5  mr-2"><i class="fa fa-check"></i> Validé</button>
            </form>
            <form method="POST" action="{{route('Conge.nonfavorable', $conge->id)}}" enctype="multipart/form-data">
                @csrf
                {{method_field('PUT')}}
                <button type="submit" class="btn  btn-danger  waves-effect waves-light m-b-5">
                 <b>X</b> Refusé
